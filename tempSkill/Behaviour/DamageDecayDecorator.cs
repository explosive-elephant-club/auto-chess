    ...
    public override void AddDMGToTarget(ChampionController target)
    {
        if (!string.IsNullOrEmpty(skill.skillData.damageData[0].type))
        {
            if (skill.selectorResult.targets[0] == target)
                skill.owner.championCombatController.TakeDamage(target, skill.skillData.damageData, 1);
            else
            {
                float dis = skill.selectorResult.targets[0].GetDistance(target);
                skill.owner.championCombatController.TakeDamage(target, skill.skillData.damageData, Mathf.Pow(damageDecay, dis));
            }
        }
    }
}
